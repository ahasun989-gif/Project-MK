...
def main_kb(lang="bn"):
    ...
    if lang=="en":
        kb.row(types.KeyboardButton("🤖 Bot Services",style="primary"),types.KeyboardButton("💰 Payments",style="success"))
        kb.row(types.KeyboardButton("👥 Referral & Earn",style="danger"),types.KeyboardButton("💬 Support",style="primary"))
        kb.row(types.KeyboardButton("VPN🛡️ Shops 🛍️",style="success"))
    else:
        kb.row(types.KeyboardButton("🤖 বট সেবা",style="primary"),types.KeyboardButton("💰 পেমেন্ট",style="success"))
        kb.row(types.KeyboardButton("👥 রেফারেল ও আয়",style="danger"),types.KeyboardButton("💬 সাপোর্ট",style="primary"))
        kb.row(types.KeyboardButton("VPN🛡️ Shops 🛍️",style="success"))
    return kb
...
def back_cancel_kb(lang="bn", back_label=None):
    ...
    kb.row(types.KeyboardButton(back,style="primary"), types.KeyboardButton(cancel,style="danger"))
    return kb

def bot_services_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    if lang=="en":
        kb.row(types.KeyboardButton("🤖 Host New Bot",style="success"),types.KeyboardButton("🛠️ Build a Bot",style="primary"))
        kb.row(types.KeyboardButton("📦 My Active Bots",style="danger"),types.KeyboardButton("📋 Price List",style="success"))
    else:
        kb.row(types.KeyboardButton("🤖 নতুন বট হোস্ট",style="success"),types.KeyboardButton("🛠️ বট বানান",style="primary"))
        kb.row(types.KeyboardButton("📦 আমার বটগুলো",style="danger"),types.KeyboardButton("📋 মূল্য তালিকা",style="success"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"))
    return kb

def payment_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    if lang=="en":
        kb.row(types.KeyboardButton("💳 Add Balance",style="success"),types.KeyboardButton("💸 Withdraw",style="danger"))
        kb.row(types.KeyboardButton("👛 My Wallet",style="primary"),types.KeyboardButton("📊 Transaction History",style="success"))
        kb.row(types.KeyboardButton("🎁 Use Coupon",style="danger"),types.KeyboardButton("📅 Subscription History",style="primary"))
    else:
        kb.row(types.KeyboardButton("💳 ব্যালেন্স যোগ",style="success"),types.KeyboardButton("💸 উইথড্র",style="danger"))
        kb.row(types.KeyboardButton("👛 আমার ওয়ালেট",style="primary"),types.KeyboardButton("📊 লেনদেন ইতিহাস",style="success"))
        kb.row(types.KeyboardButton("🎁 কুপন ব্যবহার",style="danger"),types.KeyboardButton("📅 সাব. ইতিহাস",style="primary"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"))
    return kb

def referral_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    if lang=="en": kb.row(types.KeyboardButton("🔗 Referral Panel",style="success"),types.KeyboardButton("📅 Daily Check-in",style="primary"))
    else:          kb.row(types.KeyboardButton("🔗 রেফারেল প্যানেল",style="success"),types.KeyboardButton("📅 দৈনিক চেক-ইন",style="primary"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"))
    return kb
...
def support_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    if lang=="en":
        kb.row(types.KeyboardButton("💬 Send Support",style="primary"),types.KeyboardButton("👤 My Profile",style="success"))
        kb.row(types.KeyboardButton("🌐 Change Language",style="danger"))
    else:
        kb.row(types.KeyboardButton("💬 সাপোর্ট পাঠান",style="primary"),types.KeyboardButton("👤 আমার প্রোফাইল",style="success"))
        kb.row(types.KeyboardButton("🌐 ভাষা পরিবর্তন",style="danger"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"))
    return kb
...
def pay_method_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(types.KeyboardButton("📱 Bkash",style="primary"),types.KeyboardButton("📱 Nagad",style="success"))
    kb.row(types.KeyboardButton("💱 USDT (BEP-20)",style="danger"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"), types.KeyboardButton("❌ বাতিল" if lang=="bn" else "❌ Cancel",style="danger"))
    return kb

def withdraw_method_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(types.KeyboardButton("📱 Bkash WD",style="primary"),types.KeyboardButton("📱 Nagad WD",style="success"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"), types.KeyboardButton("❌ বাতিল" if lang=="bn" else "❌ Cancel",style="danger"))
    return kb

def price_cat_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    if lang=="bn": kb.row(types.KeyboardButton("💻 হোস্টিং প্যাকেজ",style="primary"),types.KeyboardButton("🤖 কাস্টম বট মূল্য",style="success"))
    else:          kb.row(types.KeyboardButton("💻 Hosting Packages",style="primary"),types.KeyboardButton("🤖 Custom Bot Pricing",style="success"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"))
    return kb
...
def hosting_plan_kb(lang="bn"):
    ...
    for key,p in get_all_plans().items():
        kb.add(types.KeyboardButton(f"📦 {p['name']} — {int(p['price'])} BDT",style=styles[idx%3]))
        idx+=1
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"), types.KeyboardButton("❌ বাতিল" if lang=="bn" else "❌ Cancel",style="danger"))
    return kb
...
def lang_kb():
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(types.KeyboardButton("🇧🇩 বাংলা",style="primary"),types.KeyboardButton("🇬🇧 English",style="success"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু",style="primary"))
    return kb

def submit_txid_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(types.KeyboardButton("🔑 ট্রানজেকশন আইডি দিন" if lang=="bn" else "🔑 Submit Transaction ID",style="primary"))
    kb.row(types.KeyboardButton("❌ বাতিল" if lang=="bn" else "❌ Cancel",style="danger"))
    return kb

def renew_kb(lang="bn"):
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(types.KeyboardButton("🔄 নবায়ন করুন" if lang=="bn" else "🔄 Renew",style="success"),
           types.KeyboardButton("🚀 অটো-নবায়ন" if lang=="bn" else "🚀 Auto-Renew",style="primary"))
    kb.row(types.KeyboardButton("↩️ মূল মেনু" if lang=="bn" else "↩️ Main Menu",style="primary"))
    return kb
...
def admin_main_kb():
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(types.KeyboardButton("📊 ড্যাশবোর্ড",style="primary"),types.KeyboardButton("👥 ইউজার ম্যানেজ",style="success"))
    kb.row(types.KeyboardButton("💳 পেমেন্ট ম্যানেজ",style="danger"),types.KeyboardButton("🤖 বট ম্যানেজ",style="primary"))
    kb.row(types.KeyboardButton("⚙️ সেটিংস",style="success"))
    kb.row(types.KeyboardButton("↩️ ইউজার প্যানেল",style="primary"))
    return kb
...
def admin_sub_kb(*rows, back="↩️ অ্যাডমিন মেনু"):
    ...
    for row in rows:
        ...
        for idx,btn_text in enumerate(row):
            colored_row.append(types.KeyboardButton(btn_text,style=styles[idx%3]))
        kb.row(*colored_row)
    kb.row(types.KeyboardButton(back,style="primary"), types.KeyboardButton("❌ বাতিল",style="danger"))
    return kb
...
def moderator_kb():
    kb=types.ReplyKeyboardMarkup(resize_keyboard=True)
    kb.row(types.KeyboardButton("📁 হোস্টিং রিকু.",style="primary"),types.KeyboardButton("🤖 কাস্টম অর্ডার",style="success"))
    kb.row(types.KeyboardButton("🎫 সাপোর্ট টিকেট",style="danger"))
    kb.row(types.KeyboardButton("↩️ ইউজার প্যানেল",style="primary"))
    return kb
...
def feature_toggle_inline():
    ...
    kb.add(types.InlineKeyboardButton(f"🔧 Maintenance: {s('maintenance_mode')}",callback_data="tog_maint",style="primary"))
    kb.add(types.InlineKeyboardButton(f"💻 Hosting: {s('feature_hosting')}",callback_data="tog_host",style="success"))
    kb.add(types.InlineKeyboardButton(f"🤖 Custom Bot: {s('feature_custom_bot')}",callback_data="tog_cbot",style="danger"))
    kb.add(types.InlineKeyboardButton(f"👥 Referral: {s('feature_referral')}",callback_data="tog_ref",style="primary"))
    kb.add(types.InlineKeyboardButton(f"💸 Withdraw: {s('feature_withdraw')}",callback_data="tog_wd",style="success"))
    kb.add(types.InlineKeyboardButton("❌ Close",callback_data="close_panel",style="danger"))
    return kb
...
def plan_manage_inline():
    ...
    idx=0
    for key,p in get_all_plans().items():
        kb.add(types.InlineKeyboardButton(f"✏️ {p['name']} ({int(p['price'])}/{p['days']}d)",callback_data=f"editplan_{key}",style=styles[idx%3]))
        idx+=1
    kb.add(types.InlineKeyboardButton("➕ নতুন প্ল্যান",callback_data="addplan",style="success"))
    kb.add(types.InlineKeyboardButton("🗑️ প্ল্যান মুছুন",callback_data="delplan",style="danger"))
    kb.add(types.InlineKeyboardButton("❌ Close",callback_data="close_panel",style="danger"))
    return kb

def targeted_bc_inline():
    kb=types.InlineKeyboardMarkup()
    kb.row(types.InlineKeyboardButton("👥 All",callback_data="tbc_all",style="primary"),
           types.InlineKeyboardButton("⭐ Premium",callback_data="tbc_premium",style="success"))
    kb.row(types.InlineKeyboardButton("💰 Has Balance",callback_data="tbc_balance",style="danger"),
           types.InlineKeyboardButton("🆕 New(7d)",callback_data="tbc_new",style="primary"))
    kb.add(types.InlineKeyboardButton("🎯 Specific User",callback_data="tbc_userid",style="success"))
    kb.add(types.InlineKeyboardButton("❌ Cancel",callback_data="close_panel",style="danger"))
    return kb
...
def limit_settings_inline():
    ...
    styles=["primary","success","danger"]
    for idx,(label,cb) in enumerate(items): kb.add(types.InlineKeyboardButton(label,callback_data=cb,style=styles[idx%3]))
    kb.add(types.InlineKeyboardButton("❌ Close",callback_data="close_panel",style="danger"))
    return kb

def bc_type_inline():
    kb=types.InlineKeyboardMarkup()
    kb.row(types.InlineKeyboardButton("📝 Text",callback_data="bc_text",style="primary"),
           types.InlineKeyboardButton("📸 Photo",callback_data="bc_photo",style="success"))
    kb.row(types.InlineKeyboardButton("🎬 Video",callback_data="bc_video",style="danger"),
           types.InlineKeyboardButton("📄 Document",callback_data="bc_doc",style="primary"))
    kb.add(types.InlineKeyboardButton("❌ Cancel",callback_data="close_panel",style="danger"))
    return kb
...
def pay_action_inline(uid,pid,amount):
    kb=types.InlineKeyboardMarkup()
    kb.row(types.InlineKeyboardButton("✅ Approve",callback_data=f"apay_{uid}_{pid}_{amount}",style="success"),
           types.InlineKeyboardButton("❌ Reject",callback_data=f"rpay_{uid}_{pid}",style="danger"))
    return kb

def wd_action_inline(uid,wid,amount):
    kb=types.InlineKeyboardMarkup()
    kb.row(types.InlineKeyboardButton("✅ Sent",callback_data=f"wd_done_{uid}_{wid}_{amount}",style="success"),
           types.InlineKeyboardButton("❌ Cancel",callback_data=f"wd_cancel_{uid}_{wid}_{amount}",style="danger"))
    return kb

def file_review_inline(uid,rid):
    kb=types.InlineKeyboardMarkup()
    kb.row(types.InlineKeyboardButton("✅ Accept",callback_data=f"haccept_{uid}_{rid}",style="success"),
           types.InlineKeyboardButton("❌ Cancel",callback_data=f"hcancel_{uid}_{rid}",style="danger"))
    return kb

def custom_order_inline(uid,oid):
    kb=types.InlineKeyboardMarkup()
    kb.row(types.InlineKeyboardButton("✅ Accept",callback_data=f"caccept_{uid}_{oid}",style="success"),
           types.InlineKeyboardButton("❌ Reject",callback_data=f"creject_{uid}_{oid}",style="danger"))
    kb.add(types.InlineKeyboardButton("💬 Reply",callback_data=f"creply_{uid}_{oid}",style="primary"))
    return kb

def plan_confirm_inline(uid,pkey,rid):
    kb=types.InlineKeyboardMarkup()
    kb.row(types.InlineKeyboardButton("✅ Approve",callback_data=f"pok_{uid}_{pkey}_{rid}",style="success"),
           types.InlineKeyboardButton("❌ Reject",callback_data=f"prej_{uid}_{pkey}_{rid}",style="danger"))
    return kb

def support_reply_inline(uid,tid):
    kb=types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("💬 Reply",callback_data=f"rticket_{uid}_{tid}",style="primary"))
    return kb

def user_reply_inline(admin_id):
    kb=types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("💬 Reply",callback_data=f"user_reply_{admin_id}",style="primary"))
    return kb

def profile_action_inline(uid,banned,premium):
    kb=types.InlineKeyboardMarkup()
    kb.row(
        types.InlineKeyboardButton("✅ Unban" if banned else "🚫 Ban", callback_data=f"ai_unban_{uid}" if banned else f"ai_ban_{uid}",style="success" if banned else "danger"),
        types.InlineKeyboardButton("👤 Remove Premium" if premium else "⭐ Give Premium", callback_data=f"pm_toggle_{uid}",style="danger" if premium else "success")
    )
    kb.add(types.InlineKeyboardButton("💰 Add Balance",callback_data=f"addbal_{uid}",style="primary"))
    kb.add(types.InlineKeyboardButton("💬 Send Message",callback_data=f"dm_{uid}",style="success"))
    kb.add(types.InlineKeyboardButton("❌ Close",callback_data="close_panel",style="danger"))
    return kb
...
def handle_text(msg):
    try:
        ...
        if text=="VPN🛡️ Shops 🛍️":
            ...
            ikb.add(types.InlineKeyboardButton("🛍️ Go To Store",url=gs("vpn_store_link","http://t.me/vpn_store_primumea_apps_bot"),style="success"))
        ...
        if text=="🚫 ব্যান/আনব্যান":
            ...
            kb_ban.row(types.InlineKeyboardButton("🚫 Ban",callback_data="ai_ban",style="danger"),types.InlineKeyboardButton("✅ Unban",callback_data="ai_unban",style="success"))
            kb_ban.add(types.InlineKeyboardButton("❌ Cancel",callback_data="close_panel",style="danger"))
        ...
        if text=="📱 পেমেন্ট নম্বর":
            ...
            kb_num.add(types.InlineKeyboardButton(f"📱 Bkash: {gs('bkash_number')}",callback_data="editlimit_bkash",style="primary"))
            kb_num.add(types.InlineKeyboardButton("✏️ Bkash নিয়ম এডিট",callback_data="editrule_bkash",style="success"))
            kb_num.add(types.InlineKeyboardButton(f"📱 Nagad: {gs('nagad_number')}",callback_data="editlimit_nagad",style="primary"))
            kb_num.add(types.InlineKeyboardButton("✏️ Nagad নিয়ম এডিট",callback_data="editrule_nagad",style="success"))
            kb_num.add(types.InlineKeyboardButton("💱 USDT Wallet",callback_data="editlimit_usdt",style="danger"))
            kb_num.add(types.InlineKeyboardButton("✏️ USDT নিয়ম এডিট",callback_data="editrule_usdt",style="primary"))
            kb_num.add(types.InlineKeyboardButton("❌ Close",callback_data="close_panel",style="danger"))
        ...
        if text=="🛡️ VPN Store সেটিংস":
            ...
            kb_vpn.add(types.InlineKeyboardButton(f"🔗 লিংক: {gs('vpn_store_link')}",callback_data="editlimit_vpnlink",style="primary"))
            kb_vpn.add(types.InlineKeyboardButton("✏️ বার্তা এডিট করুন",callback_data="editlimit_vpntext",style="success"))
            kb_vpn.add(types.InlineKeyboardButton("❌ Close",callback_data="close_panel",style="danger"))
...
def _send_coupon_panel(uid):
    ...
    kb=types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("➕ নতুন কুপন তৈরি",callback_data="coupon_create",style="success"))
    kb.add(types.InlineKeyboardButton("❌ Close",callback_data="close_panel",style="danger"))
